...
def sample_discrete_features(probX, probE, node_mask):
    ''' Sample features from multinomial distribution with given probabilities (probX, probE, proby)
        ...
        :param proby: bs, dy_out           label features.
    '''
    bs, n, _ = probX.shape
    # Noise X
...
def sample_discrete_feature_noise(limit_dist, node_mask):
    ...
    #这里y_limit.flatten()是将y_limit默认展平为一维，不需要设置end_dim
    U_y = y_limit.flatten().multinomial(1).reshape(bs, 1)
    #这里y为空，没有对其进行采样，需要修改！！！
    #U_y = torch.empty((bs, 0))
    #以上让x,e,y中每个元素都是一个概率分布,如[0.3,0.3,0.4]
